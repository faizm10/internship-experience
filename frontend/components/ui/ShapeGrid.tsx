...
export default function ShapeGrid({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // subtle center fade so the grid feels atmospheric
    ...
    const updateHoveredFromClient = (clientX: number, clientY: number) => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      updateHoveredFromClient(event.clientX, event.clientY)
    }

    const handleTouchMove = (event: TouchEvent) => {
      const t = event.touches[0]
      if (!t) return
      updateHoveredFromClient(t.clientX, t.clientY)
    }
