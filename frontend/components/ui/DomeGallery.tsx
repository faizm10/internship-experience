...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const enlargedOverlay = viewerRef.current?.querySelector(`.${styles.enlarge}`) as HTMLDivElement | null
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const overlay = viewerRef.current?.querySelector(`.${styles.enlarge}`) as HTMLDivElement | null
      if (!overlay || !parent) return

      const refDiv = parent.querySelector(`.${styles.itemImageReference}`) as HTMLDivElement | null
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      if (mode === "dome") {
        ...
        tileR = refDiv.getBoundingClientRect()
      } else {
        parent.style.setProperty("--rot-y-delta", "0deg")
        parent.style.setProperty("--rot-x-delta", "0deg")
        tileR = el.getBoundingClientRect()
      }
      ...
      if (!mainR || !frameR || tileR.width <= 0 || tileR.height <= 0) {
        openingRef.current = false
        focusedElRef.current = null
        // best-effort cleanup for dome-only ref div
        const refDiv = parent.querySelector(`.${styles.itemImageReference}`)
        refDiv?.remove()
        unlockScroll()
        return
      }
      ...
      // After the initial expand, resize to best-fit viewport based on image aspect ratio.
      const fitToViewport = () => {
        const mainR2 = mainRef.current?.getBoundingClientRect()
        if (!mainR2) return

        const pad = Math.max(12, Math.round(Math.min(window.innerWidth, window.innerHeight) * 0.045))
        ...
        const left = window.innerWidth / 2 - targetW / 2 - mainR2.left
        const top = window.innerHeight / 2 - targetH / 2 - mainR2.top

        // Animate position+size (keep transform at identity)
        ...
        requestAnimationFrame(() => {
          ...
          overlay.style.width = `${Math.round(targetW)}px`
          overlay.style.height = `${Math.round(targetH)}px`
        })
      }

      if (img.complete) {
        window.setTimeout(fitToViewport, enlargeTransitionMs + 40)
      } else {
    ...
    [enlargeTransitionMs, lockScroll, mode, segments, unlockScroll]
  )
    ...
    >
      <main ref={mainRef} className={styles.sphereMain}>
        {mode === "dome" ? (
          ...
          <div className={styles.flatGrid} aria-label="Photo gallery">
            {images.map((imgItem, i) => {
              const normalized =
                typeof imgItem === "string" ? { src: imgItem, alt: "" } : imgItem
              return (
                <div
                  key={`${normalized.src}-${i}`}
                  className={styles.flatTile}
                  data-src={normalized.src}
                  role="button"
                  tabIndex={0}
                  aria-label={normalized.alt || "Open image"}
                  onClick={(e) => {
                    if (openingRef.current) return
                    openItemFromElement(e.currentTarget)
                  }}
                ...
                >
                  <img src={normalized.src} draggable={false} alt={normalized.alt || ""} />
                </div>
              )
            })}
          </div>
